# HSWare Post — Public Submission v3.0
...
## Tools
1. `get_hsware_contract` — read-only; returns HSWare rules interpreted against a runtime prompt.
2. `validate_hsware_json` — read-only; validates candidate JSON and returns validation findings.
3. `hsware_health` — read-only diagnostic.

## Before submission
- Deploy v3.0 and verify `/health`, `/privacy`, `/terms`, `/support`, and `/mcp`.
