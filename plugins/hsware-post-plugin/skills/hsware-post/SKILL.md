...
## HSWare Post Plugin v1.0 Validation Hardening

These rules strengthen the existing contract and do not replace runtime requirements.

### Focus Keyword Density Gate

When the runtime prompt requires focus-keyword density validation:
- treat the supplied focus keyword as the exact-match phrase
- count matches case-insensitively in the HSWare-validatable generated article text
- compute density as: exact focus-keyword occurrences / total validated article words * 100
- target 1.0%-1.2% for a conservative safety margin
- never intentionally return content above 2.0%, even when the runtime hard ceiling is 2.2%
- preserve the runtime hard range when it is stricter than these defaults
- if density is above target, replace unnecessary exact matches with natural contextual alternatives such as the app, the software, the platform, the browser, the tool, or a pronoun when context permits
- if density is below the runtime minimum, add natural exact-match usage only where useful
- recalculate after every correction
- never merely estimate or claim that density passes

Example: 23 exact matches in 900 validated words = 2.56%, which must be reduced before output.

### Feature Description Gate

For every feature object:
- count the feature description independently
- when FAQ is enabled, never return a description below 35 words and normally target 45-60 words unless runtime rules specify otherwise
- when FAQ is disabled, obey the runtime's longer feature-description requirement
- if one feature fails, rewrite only that feature first, then rerun all final checks

### Strict JSON Parse Gate

Before returning the final object:
- ensure the entire response is one complete JSON object
- ensure every string is terminated
- escape embedded double quotes and backslashes correctly
- never place raw line breaks inside JSON strings
- reject trailing commas, missing commas, malformed escapes, truncated arrays/objects, or incomplete braces/brackets
- run a full-object parse check when an execution environment is available
- if parsing fails, repair and parse again before returning

### Final Repair Loop

Before output, run all applicable HSWare checks together. If any check fails, repair only the failing field or density issue when possible, then rerun the complete validation set. Do not return a known-invalid draft.
